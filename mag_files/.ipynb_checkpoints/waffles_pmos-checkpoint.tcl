...
load waffle_cells/pmos_waffle_corners.mag
save mag_files/pmos_waffle_48x48
load mag_files/pmos_waffle_48x48
### corners ###
...
for {set i 0} {$i < $n - 2} {incr i} { #ancho
    for {set j 0} {$j < $n - 2} {incr j} { #alto
		...
		if {[expr {($i + $j)%2}]} {
			getcell waffle_cells/pmos_drain_in.mag
		} else {
			getcell waffle_cells/pmos_source_in.mag
		}
	}
}
...
label PW
port make 4
save mag_files/POSTLAYOUT/pmos_flat_48x48
extract all
ext2sim labels on
ext2sim
